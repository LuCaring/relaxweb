...
class RewardsTests(unittest.TestCase):
    ...
    def draw(self, request_id="draw-0001", name="alice", now=NOW):
        return self.transaction(rewards.draw_lottery, name, request_id, now, wallet_module.adjust_coins)
    ...
    def test_handlers_ignore_client_prize_day_and_target(self):
        async def run():
            with patch.object(server.hub, "send_json", new_callable=AsyncMock) as send:
                await server.rewards.handle_draw_lottery(None, {}, {"request_id": "draw-0001"})
                ...
                with patch.object(time, "time", return_value=NOW):
                    await server.rewards.handle_daily_checkin(None, state, {"day": "2030-01-01", "tickets": 999})
                    self.assertEqual(send.call_args.args[1]["awarded"], 5)
                    with patch.object(rewards, "pick_prize", return_value=20):
                        await server.rewards.handle_draw_lottery(None, state,
    ...
    def test_updates_only_go_to_same_account_connections(self):
        async def run():
            self.checkin()
            server.hub.clients.update({"tab1": {"user": {"username": "alice"}},
                                   "tab2": {"user": {"username": "alice"}},
                                   "other": {"user": {"username": "bob"}}})
            with patch.object(server.hub, "send_json", new_callable=AsyncMock) as send:
                await server.rewards.handle_draw_lottery("tab1", server.hub.clients["tab1"], {"request_id": "draw-0001"})
