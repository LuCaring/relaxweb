...
class ApplicationTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_route_contract_is_complete(self):
        expected = """register login resume logout update_profile get_profile delete_account get_online
            ...
            estate_market_get estate_market_trade estate_market_order estate_market_cancel
            estate_penguin estate_lottery_draw estate_use_land_upgrade_ticket
            estate_lottery_history estate_select_pet
            voice_hub_join voice_hub_leave voice_hub_rename voice_hub_chat voice_hub_state voice_hub_unsubscribe"""
        self.assertEqual(set(self.app.handlers), set(expected.split()))
        self.assertEqual(len(self.app.handlers), 82)
