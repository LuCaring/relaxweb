...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('.estate-skin-card').count(), 3);
    assert.equal(await page.locator('.estate-equip').isDisabled(), true);
    await page.locator('[data-skin-id="xiaopang"]').click();
    ...
    assert.equal(request.skin_id, 'xiaopang');
    assert.equal(await page.locator('.estate-equip').isDisabled(), true);
    assert.match(await page.evaluate(() => spriteDraws.at(-1).src), /\/berry\//, 'map waits for server confirmation');
    await respond(page, 'xiaopang', { request_id: request.request_id, result: { action: 'set_skin', skin_id: 'xiaopang', replayed: false } });
    await page.waitForFunction(() => document.querySelector('.estate-wardrobe-status').textContent.includes('已保存到账号'));
    await page.waitForFunction(() => spriteDraws.at(-1).src.includes('/xiaopang/'));
    assert.equal(await page.locator('[data-skin-id="xiaopang"] .estate-skin-badge').innerText(), '已穿戴');
    ...
    await respond(page, 'rose_mage', { result: { action: 'set_skin', skin_id: 'xiaopang', replayed: true } });
    await page.waitForFunction(() => spriteDraws.at(-1).src.includes('/rose_mage/'));
    assert.equal(await page.locator('[data-skin-id="rose_mage"] .estate-skin-badge').innerText(), '已穿戴');
    ...
    const persisted = { ...fixture, profile: { ...fixture.profile, skin_id: 'rose_mage' } };
    ...
    assert.equal(await failure.evaluate(async () => (await import('/assets/js/estate/state.js')).estateStore.pending.size), 0);
