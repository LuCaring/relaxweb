...
function rewardLabel(result) {
  ...
  if (award === "fertilizer_5") return "恭喜获得化肥 ×5！";
...
export function renderLotteryGame(target, snapshot, onBusyChange) {
  ...
  note.textContent = `8 个奖项等概率。每次 ${price.toLocaleString("zh-CN")} 金币；神秘大奖另按公布概率结算，其中“再抽一次”免费。抽奖前须留出 5 格仓位。`;
