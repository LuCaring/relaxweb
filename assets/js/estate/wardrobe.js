...
export function createEstateWardrobe(root, { onOpen, onClose } = {}) {
  ...
  layer.innerHTML = `
    <section role="dialog" aria-modal="true" aria-labelledby="estate-wardrobe-title" tabindex="-1">
      ...
      <div class="estate-wardrobe-body">
        ...
        <div class="estate-wardrobe-collection"><div class="estate-collection-title"><b>我的皮肤</b><span>全部免费 · 随心换装</span></div><div class="estate-skin-list" role="group" aria-label="可选角色皮肤"></div><p class="estate-wardrobe-note">只换心情，不改属性。<br>换装后自动保存到当前账号。</p></div>
      </div>
  ...
  function update() {
    ...
    for (const id of CHARACTER_IDS) {
      ...
      card.querySelector(".estate-skin-badge").textContent = wearing ? "已穿戴" : "试穿";
    }
    layer.querySelector(".estate-selected-name").textContent = catalog[selected]?.name || "角色衣橱";
    equip.disabled = saving || !loaded || !snapshot || !Object.hasOwn(catalog, selected) || selected === equipped();
    equip.textContent = saving ? "正在保存…" : selected === equipped() ? "正在穿戴" : "穿上这套";
  ...
  function select(id, retry = false) {
    ...
    status.textContent = selected === equipped() ? "这是你当前穿戴的皮肤" : "试穿中，点击「穿上这套」保存";
    update();
  ...
  function open() {
    if (!estateStore.snapshot || !layer.hidden) return;
  ...
  equip.addEventListener("click", async () => {
    ...
    const id = selected; saving = true; status.textContent = "正在把新装扮保存到账号…"; update();
    try {
  ...
  const unsubscribe = subscribeEstate(() => { if (!estateStore.snapshot) hide(); update(); });
  update();
  return {
    open,
