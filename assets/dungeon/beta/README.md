...
- `player/idle-source.png`: five rows, six columns; `move-source.png`: five rows, eight columns. The player is intentionally a tomato. Both sheets have uneven generated cell dimensions, so the prototype crops cells proportionally. The row mapping is provisional and should be checked visually before production use.
- `enemies/cave_bat/` and `enemies/stone_brute/`: each action sheet is 2172 × 724, six frames of 362 × 724. `frames.json` records the four actions. The source frames contain generous transparent padding.
