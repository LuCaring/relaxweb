...
日期：2026-09-24。用途：A/B/C三线开始工作时共同对齐的合同。**本文示例是拟实现接口，不是当前服务端支持的API；JSON可解析不代表对应schema、服务或插件已经实现。** A在F0把这些结构落到`contracts/dungeon/schemas`与fixture后，标记正式合同版本。实现分工见[开发指南](dungeon-beta-development-guide.md)，可靠性设计见[架构方案](dungeon-beta-architecture.md)。

2026-09-25 实现更新：已落地内容包schema、Simulator Protocol及护盾/升级两个fixture，详见[可执行合同说明](../contracts/dungeon/README.md)与[交接记录](dungeon-beta-implementation-status.md)。下文完整消息协议尚未注册为公开服务，以实际代码为准。
...
```json
{
  ...
  "upgrade_level": 2,
  "stats": {"attack": 24},
  "affixes": [{"id": "beta.affix.haste", "roll": {"attack_speed_bp": 400}}],
  "location": "bag",
  "locked": false,
  "trade": {"allowed": true, "reason": null},
  "reservation": null
}
```

这是装备公开视图；正式存储另含owner、来源、绑定、实例版本与效果配置快照。客户端不得根据`trade.allowed`跳过服务端检查。`reservation`非空时至少给purpose/ref和可操作提示；物品预留本身由独立记录约束，变更可用性会增加拥有者资产revision。
...
| 请求名 | 关键输入 | 返回/作用 |
| --- | --- | --- |
| `dungeon_beta_get_state` | request_id | 营地资产版本、钱包、背包/待领取、成长、当前run、可用能力 |
| `dungeon_beta_get_catalog` | request_id、已知ruleset_id可选 | 经裁剪的公开内容、规则版本、可显示效果说明 |
| `dungeon_beta_equip` | request_id、slot、item_id、expected_asset_revision | 经预留检查后穿戴；item_id=null表示卸下 |
| `dungeon_beta_lock_item` | request_id、item_id、locked、expected_item_version | 设置锁定；不是toggle |
| `dungeon_beta_claim_items` | request_id、item_ids、expected_asset_revision | 满容量整单拒绝，不默默部分领取 |
...
| `dungeon_beta_list_offers` | request_id、cursor可选 | 当前用户作为买家/卖家的报价与已成交结果，不泄露其它私密订单 |
...
完整API仍需补分解、制作等实际启用的动作；没有启用的功能不发空按钮。交易手续费/TTL/上限的公开策略通过get_catalog返回；用户确认页面据此展示，服务端在create_offer时重算，策略变化返回`quote_changed`，不能用已变价费率自动创建报价。
...
move/aim分量暂定−1024～1024，原点aim沿上一次有效朝向；move规范化保证斜向不加速。buttons采用schema列出的有限布尔字段；闪避/武技按输入序号识别新按下，不因重复包反复触发。服务端时间权威，不按客户端声称的经过时间推进世界。
...
`services`只包含宿主提供的确定性随机流、空间查询基础能力和事件收集器，不含数据库、wallet、网络连接或系统时钟。step返回新状态/状态变化、战斗事件与可验证房间结束信号；不能返回任意“金币增加X”命令。
...
fixture由正式schema校验后提交，B使用同一JSON做假服务，C使用同一规则集做样例模拟；不能三组各写一种“差不多”的字段。F0至少包含一个从开始到结束的完整正常路径和资金不足、资产预留、响应丢失三条失败路径。
