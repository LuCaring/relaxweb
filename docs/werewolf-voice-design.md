...
| 夜晚狼队协商 | 狼队 | 狼人 | 仅狼人 + 了结死亡的旁观者（服务端只授订阅权） |
| 出局后（遗言/开枪未了） | 死者频道 | 死者互聊 | 全员不适用；身份保密 |
| 出局后（了结，上帝视角） | 白天 day / 夜晚狼队 | 禁言（不得再向对局发言） | 全场身份公开 + 狼频道可听 |
...
- **死亡两段式（2026-09-27 修订）**：遗言/猎人开枪未了结的死者仍在局中——
  身份保密、走死者频道；了结即转上帝视角旁观：视图带 `god_view`（全场
  身份公开），夜晚 `voice_view` 给 wolf 频道（token 只授订阅权，服务端
  强制禁言），文字上可读狼频道与死者频道但 `chat_route` 返回 `""` 拦截
  一切发言。顺带修复：被放逐/夜间死亡的猎人在 shot 阶段此前拿不到
  `your_options`（`_your_options` 先按存活过滤导致死代码），现在
  shot 行动方优先于存活检查，猎人可以真正从 UI 开枪。
